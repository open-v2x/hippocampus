...
if [ "$SOURCE_URL" != "" ]; then
   ...
   rtsp-simple-server /config/rtsp-simple-server.yml &
   sleep 2

   echo "Start relaying from $SOURCE_URL to rtsp://0.0.0.0:8554/$STREAM_NAME"
   while true; do
      set -x
      ffmpeg $FFMPEG_INPUT_ARGS -re -stream_loop -1 -i $SOURCE_URL $FFMPEG_OUTPUT_ARGS $RTSP_PROXY_SOURCE_TCP -preset ultrafast -f rtsp rtsp://0.0.0.0:8554/$STREAM_NAME
      set +x
      echo "Reconnecting..."
      sleep 1
   done
